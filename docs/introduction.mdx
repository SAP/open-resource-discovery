...
The [SAP Business Accelerator Hub](https://api.sap.com/) is an example of an ORD aggregator that uses static information to present generic documentation.
...
The Unified Customer Landscape is foremost a technical background service that offers the system landscape metadata (incl. ORD information) to other applications and services.
While the [Business Accelerator Hub](#business-accelerator-hub) focuses on the static perspective of a product, the UCL can provide the actual system tenant metadata - including possible customer extensions and customizations.

This is also exposed via the [SAP BTP](https://www.sap.com/products/technology-platform.html) Cockpit in the System Landscape overview.
...
The most notable concept is the [ORD System Instance](./index.md#def-system-instance), which is usually a tenant.
